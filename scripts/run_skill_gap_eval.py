...
from natural_reward_seeking.config import load_skill_gap_eval_config
...
from natural_reward_seeking.prompting import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the skill-gap evaluation comparing naive task engagement, intrinsic alignment, "
            "and reward-reasoning prompting."
        )
    )
    parser.add_argument("--config", default="configs/skill_gap_eval.toml")
...
def main() -> None:
    args = parse_args()
    config = apply_overrides(load_skill_gap_eval_config(args.config), args)
    ...
    if "generation" in stages:
        ...
        manifest.update(
            {
                ...
                "num_prompt_conditions": len(SKILL_GAP_PROMPT_CONDITIONS),
                "num_total_cases": len(sampled_rows) * len(SKILL_GAP_PROMPT_CONDITIONS),
                "evaluation_type": "skill_gap",
            }
        )
        ...
        case_rows = build_case_rows(sampled_rows, conditions=SKILL_GAP_PROMPT_CONDITIONS)
        ...
        response_rows = generator.generate_case_rows(
            case_rows,
            progress_desc="Generating skill-gap responses",
        )
        generator.release()
    ...
    render_all_plots(
        ...
        condition_order=[condition.name for condition in SKILL_GAP_PROMPT_CONDITIONS],
        condition_labels={condition.name: condition.label for condition in SKILL_GAP_PROMPT_CONDITIONS},
        condition_colors=SKILL_GAP_CONDITION_COLORS,
    )
