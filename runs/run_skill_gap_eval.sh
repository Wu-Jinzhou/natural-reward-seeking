...
CONFIG_PATH="${CONFIG_PATH:-$ROOT_DIR/configs/skill_gap_eval.toml}"
DATASET_ROOT="${DATASET_ROOT:-$ROOT_DIR/wildjailbreak}"
SAMPLE_PER_CATEGORY="${SAMPLE_PER_CATEGORY:-500}"
SEED="${SEED:-42}"
POLICY_MODEL_ID="${POLICY_MODEL_ID:-allenai/Olmo-3-32B-Think-SFT}"
...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/results/skill_gap_eval/$MODEL_SLUG}"

export VLLM_WORKER_MULTIPROC_METHOD

cmd=(
  python "$ROOT_DIR/scripts/run_skill_gap_eval.py"
  --config "$CONFIG_PATH"
...
printf 'Running skill-gap eval with model %s and output dir: %s\n' "$POLICY_MODEL_ID" "$OUTPUT_DIR"
"${cmd[@]}"
